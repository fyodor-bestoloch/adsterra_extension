...
async function extractInnerElementsWithAxios(url) {
  try {
    ...
    bodyElements.forEach(element => {
      const elementHtml = element.outerHTML.trim();
      if (elementHtml) {
...
let matches = [];
let t = 0;
async function sendHtmlElementToApi(element, elementIndex, totalElements, url) {
  ...
  const payload = {
    ...
    "total_chunks": totalElements,
    "chunk_number": elementIndex,
    "user": "not yet implemented. Stay tuned!",
    "url": url
  };
  ...
  try { // PROMISE
    ...
    res1.innerText = `  ${elementIndex}/${totalElements}`;
  } catch (error) {
...
async function processAndSendWebsiteElements(url) {
  const elements = await extractInnerElementsWithAxios(url);
  const totalElements = elements.length;
  for (let i = 0; i < totalElements; i++) { //
    await sendHtmlElementToApi(elements[i], i + 1, totalElements, url);
    console.log('1');
  } //promise
  if (matches.length === 0) {
  ...
  res2.innerText =  `Here are the results: ${matches}`;
  console.log('All elements have been sent successfully!');
}

function doTheMainThing() {

  document.getElementById('run-script').addEventListener('click', () => {
    chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {

        const container = document.getElementById('main-content');
        container.innerHTML = `<h1>Inspecting the elements... </h1>`;
        document.getElementById('res123').style.display = "block";
        const res1 = document.getElementById("res1");
        const res2 = document.getElementById("res2");
        res1.style.display = "block";
        res2.style.display = "block";
        processAndSendWebsiteElements(tabs[0].url).then(r => console.log("success"));
    });
  });

}

function getSessionUUID(callback) {
  chrome.storage.local.get(['sessionUUID'], function(result) {
    if (chrome.runtime.lastError) {
      console.error('Error retrieving session_uuid:', chrome.runtime.lastError);
      callback(null);
      return;
    }
    callback(result.sessionUUID || null);
  });
}

chrome.storage.local.get(null, async function (result) {
  if (Object.keys(result).length === 0) { // that means no relevant uuid is saved and authorization required
    console.log("chrome.storage.local is empty.");
    ...
    title1.innerHTML = "Enter Your ID";
    idField.style.display = "block";
    submitButton.style.display = "block";
    submitButton.addEventListener("click", async () => {
      const user = idField.value.trim();
      const endpoint = "https://salesops.adsterratech.com/chrome_extension/api/auth/session";
      if (!user) {
        ...
        const response = await axios.post(endpoint, {user}, {
          headers: {
            ...
            .then(response => {
              console.log("Response from code request:", response);
              ...
              let sessionUUID = response.data.session_id;
              chrome.storage.local.set({sessionUUID: sessionUUID}).then(() => {
                console.log("Value is set", sessionUUID);
              });
            })
            .catch(error => {
              if (error.response) {
                console.error("Error during code request:", error);
                responseParagraph.textContent = `Error ${error.status}: ${error.response.data.title} ${error.response.data.detail}`;
                responseParagraph.style.display = "block";
              }
            });
    });
    ...
    submitCodeButton.addEventListener("click", async () => {
      getSessionUUID(async function (sessionUUID) {
      const user = idField.value.trim();
      ...
      const endpoint2 = "https://salesops.adsterratech.com/chrome_extension/api/auth/session/verify";
      const response = await axios.post(endpoint2, payload, {
        headers: {
          ...
          .then(response => {
            ...
            title2.style.display = "none"; //hide certain elements and display other ones
          })
          .catch(error => {
            if (error.response) {
              console.error("Error during code request:", error);
  ...
  else
  {
    ...
    getSessionUUID(async function (sessionUUID) {
        const response = await axios.get(`https://salesops.adsterratech.com/chrome_extension/api/auth/session?session_uuid=${sessionUUID}`, {headers}) //заменить на get
            .then(response => { //checking whether the uuid is still valid
              console.log("200");
              console.log("321313");
              ...
              const submitCodeButton = document.getElementById("submitCodeButton");
              const responseParagraph = document.getElementById("response");
              const res = document.getElementById("res123");
            ...
            .catch(error => {
              if (error.response) {
                ...
                const title1 = document.getElementById("title1");
                title1.style.marginLeft = "20px";
                title1.style.marginRight = "20px";
                title1.textContent = "Please reopen the popup in order to reauthorize";
                console.log("All data in chrome.storage.local has been cleared.");
            //clears the local storage (only 1 record per device will be made anyway so we can just clear it all)
                });
              }
            })
    });
